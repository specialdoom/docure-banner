<svelte:options tag="docure-banner" />

<script>
  export let imagesrc = '';
  const style = `background-image: url(${imagesrc}); background-size: auto 400px; background-repeat: no-repeat`;
</script>

<div class="docure-banner-content"><slot /></div>
<div class="docure-banner-image" {style} />

<style>
  :host {
    display: flex;
    position: relative;
    width: 100%;
    height: 400px;
  }

  :host div.docure-banner-content {
    width: 70%;
    height: 100%;
    display: flex;
    flex-direction: column;
    box-sizing: border-box;
    background: #dc493a;
    padding: 10px 30px;
  }

  :host div.docure-banner-image {
    width: 30%;
  }

  div.docure-banner-image::before {
    position: absolute;
    display: block;
    content: ' ';
    top: 0;
    bottom: 0;
    right: 0;
    left: 70%;
    background-image: url('data:image/svg+xml;base64,');
    background-repeat: no-repeat;
  }
</style>
